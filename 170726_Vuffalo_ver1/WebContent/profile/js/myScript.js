...
function goServlet(nowUser, codeNum, name, type){
	//alert(nowUser+", "+codeNum +", "+name+", "+type);
	if(!isMember(nowUser)){
	...
	var flag;
	if(type==3)
		flag=confirm("친구 요청을 하시겠습니까?");
	else if(type==6)
		flag=confirm(name+'님을 차단하시겠습니까?')
	else if(type==11)
		flag=confirm(name+"님에게 편지를 보내시겠습니까?");
	else if(type==12)
		flag=confirm(name+"님을 즐겨찾기 하시겠습니까?");
	...
	if(flag==true){
		if(type==3)
			...
			location.href="linkSendMail.do?receiverNickName="+name;
		else if(type==12)
			location.href="UpdateRelation.do?p_codeNum="+codeNum+"&p_name="+name+"&type="+5;
	}
};
