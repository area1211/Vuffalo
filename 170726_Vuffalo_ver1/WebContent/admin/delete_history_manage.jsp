...
<script src="admin/js/delete_history_manage.js"></script>
<link rel="stylesheet" href="admin/css/delete_history_manage.css"/>
...
<body>
	...
	<div id="pageForm" class="paging">
        <c:if test="${startPage != 1}">
            <a href="deleteHistoryManageServlet.do?page=${startPage-1}&lNum=${lNum}">[이전]</a>
        </c:if>
        ...
        <c:forEach var="pageNum" begin="${startPage}" end="${endPage}">
            ...
            <c:if test="${pageNum != spage}">
                <a href='deleteHistoryManageServlet.do?page=${pageNum}&lNum=${lNum}'>${pageNum}&nbsp;</a>
            </c:if>
        </c:forEach>
        
        <c:if test="${endPage != maxPage }">
            <a href='deleteHistoryManageServlet.do?page=${endPage+1}&lNum=${lNum}'>[다음]</a>
        </c:if>
        <div class="btns_div">
        <input type="button" id="delete_btn" value="영구삭제"/>
        <input type="button" id="restore_btn" value="복원"/>
        </div>	
    </div><!-- 페이징 -->
	</div><!-- 전체div -->
</body>
